...
import { X, Heart, MessageSquare, Plus, ImagePlus, Loader2, Edit2, Trash2, Check } from 'lucide-react';
...
export default function Exhibition() {
  ...
  const displayedMemories = memories.slice(0, INITIAL_DISPLAY_COUNT);
  ...
  const fetchMemories = async () => {
    ...
    try {
      ...
      const mergedMemories = assetsConfig.exhibitionImages.map((img, idx) => {
        ...
        if (!dbMem) {
          // Sync base memory to DB asynchronously
          supabase.from('memories').insert({ 
            id, url: img.url, caption: img.caption, "desc": img.desc, is_user_added: false 
          }).then();
        }
  ...
  const handleLike = async (id: string) => {
    ...
    try {
      ...
      // Update total likes in memories table
  ...
  return (
    <section className="relative py-24 bg-background border-y border-border">
      {/* Texture overlay */}
      <div className="absolute inset-0 opacity-[0.03] bg-[url('https://www.transparenttextures.com/patterns/aged-paper.png')] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 mb-16 text-center relative z-10">
        <motion.h3 
          ...
          className="text-3xl md:text-5xl font-serif tracking-widest text-[#d4c3a3] mb-6"
        >
          {contentConfig.exhibition.sectionTitle}
        </motion.h3>
        <motion.p 
          initial={{ opacity: 0 }}
        ...
        >
          {contentConfig.exhibition.description}
        </motion.p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 relative z-10">
          ...
          {displayedMemories.map((item, idx) => (
            ...
            >
              {/* Frame */}
              <div className="p-4 bg-[#1a1a1a] border border-[#333] shadow-2xl transition-transform duration-500 group-hover:-translate-y-2 h-full flex flex-col">
      ...
      {/* View All Button */}
      {!isLoading && memories.length > INITIAL_DISPLAY_COUNT && (
        <motion.div 
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
        ...
        >
          <button 
            ...
            className="px-8 py-3 border border-[#d4c3a3] text-[#d4c3a3] font-serif tracking-widest hover:bg-[#d4c3a3] hover:text-black transition-colors duration-300"
          >
            查看全部记忆
          </button>
        </motion.div>
      ...
      <AnimatePresence>
        {selectedItem !== null && (
          ...
          >
            <motion.button 
              className="absolute top-4 right-4 md:top-8 md:right-8 text-[#d4c3a3] hover:text-white transition-colors z-[60]"
              onClick={() => setSelectedItem(null)}
            >
            ...
            <motion.div 
              ...
              className="w-full max-w-6xl max-h-[90vh] flex flex-col md:flex-row gap-0 bg-[#0a0a0a] border border-[#333] shadow-2xl overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Left: Image */}
